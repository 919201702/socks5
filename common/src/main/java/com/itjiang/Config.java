...
public final class Config {
    ...
    public static final boolean CLIENT_SOCKS5_AUTH_ENABLED;
    ...
    static {
        ...
        CLIENT_SOCKS5_AUTH_ENABLED = parseBoolean(props, "client.socks5.auth.enabled", false);
        ...
        if (CLIENT_SOCKS5_AUTH_ENABLED) {
            if (CLIENT_SOCKS5_USERNAME.isBlank()) {
                throw new RuntimeException("配置错误: client.socks5.username 不能为空（已开启 client.socks5.auth.enabled）");
            }
            if (CLIENT_SOCKS5_PASSWORD.isBlank()) {
                throw new RuntimeException("配置错误: client.socks5.password 不能为空（已开启 client.socks5.auth.enabled）");
            }
        }
